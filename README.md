<div align="center">
  
# 👋 你好，我是 Mark

*热衷于开发实用工具，探索AI应用，让技术服务生活*

[![GitHub followers](https://img.shields.io/github/followers/FB208?style=for-the-badge&logo=github)](https://github.com/FB208)
[![GitHub Stars](https://img.shields.io/github/stars/FB208?style=for-the-badge&logo=github)](https://github.com/FB208?tab=repositories)

</div>

---

## 🚀 关于我

- 🔨 专注于开发提升工作效率的实用工具
- 🤖 探索AI在日常工作场景中的实际应用
- 📝 Obsidian生态贡献者，简洁主义倡导者
- 🌐 WebDAV技术实践者，数据同步与备份爱好者
- 💡 相信好的工具应该简单、可靠、实用

## 💻 技术栈

![Python](https://img.shields.io/badge/-Python-3776AB?style=flat-square&logo=python&logoColor=white)
![JavaScript](https://img.shields.io/badge/-JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![C%23](https://img.shields.io/badge/-C%23-239120?style=flat-square&logo=c-sharp&logoColor=white)
![PowerShell](https://img.shields.io/badge/-PowerShell-5391FE?style=flat-square&logo=powershell&logoColor=white)
![Vue.js](https://img.shields.io/badge/-Vue.js-4FC08D?style=flat-square&logo=vue.js&logoColor=white)
![Go](https://img.shields.io/badge/-Go-00ADD8?style=flat-square&logo=go&logoColor=white)
![Rust](https://img.shields.io/badge/-Rust-000000?style=flat-square&logo=rust&logoColor=white)

---

<!-- AUTO-GENERATED-PROJECTS-START -->

## 🎯 我的所有项目

> 📊 总计 **46** 个仓库 | 原创 **19** | Fork **26**  
> 🔒 带锁标记的为私有仓库  
> 🤖 最后更新: 2025-10-09 08:49:21 (UTC+0)


### 📦 Fork

- **[NotionNext](https://github.com/FB208/NotionNext)** - 使用 NextJS + Notion API 实现的，支持多种部署方案的静态博客，无需服务器、零门槛搭建网站，为Notion和所有创作者设计。 (A static blog built with NextJS and Notion API, supporting multiple deployment options. No server required, zero threshold to set up a website. Designed for Notion and all creators.)


### 📦 Other

- **[CodeSpace](https://github.com/FB208/CodeSpace)** - 把所有非公司产权的代码都整合到一起了
- **[clash-linux-amd64-v1.2.0](https://github.com/FB208/clash-linux-amd64-v1.2.0)** - 暂无描述
- **[MarkTools](https://github.com/FB208/MarkTools)** - 暂无描述
- **[vue2-elm](https://github.com/FB208/vue2-elm)** - 基于 vue2 + vuex 构建一个具有 45 个页面的大型单页面应用
- **[vue-manage-system](https://github.com/FB208/vue-manage-system)** - The web management system solution based on Vue2 and Element UI.
- **[mytab](https://github.com/FB208/mytab)** - edge浏览器插件，通过自定义webdav来存储书签，支持历史备份，永不丢失
- **[obsidian-releases](https://github.com/FB208/obsidian-releases)** - Community plugins list, theme list, and releases of Obsidian.
- **[obsidian-simple-ai](https://github.com/FB208/obsidian-simple-ai)** - obsidian上的AI插件都太强大了，同时也太复杂了，我只需要一个像notion一样能帮我编辑文字内容的简单AI
- **[md](https://github.com/FB208/md)** - ✍ WeChat Markdown Editor | 一款高度简洁的微信 Markdown 编辑器：支持 Markdown 语法、自定义主题样式、内容管理、多图床、AI 助手等特性
- **[crush](https://github.com/FB208/crush)** - The glamourous AI coding agent for your favourite terminal 💘
- **[yibiao-simple](https://github.com/FB208/yibiao-simple)** - 开箱即用的AI标书编写工具
- **[lifeRestart](https://github.com/FB208/lifeRestart)** - やり直すんだ。そして、次はうまくやる。
- **[webdavsync](https://github.com/FB208/webdavsync)** - 通过webdav协议定期备份文件
- **[bazi](https://github.com/FB208/bazi)** - python八字排盘软件。清晰看出冲刑合会、阴阳等关系，并有凝聚大师多年经验的评判。另有合婚、风水等功能。
- **[bazi-mcp](https://github.com/FB208/bazi-mcp)** - MCP server for Bazi (八字) information
- **[Pake](https://github.com/FB208/Pake)** - 🤱🏻 Turn any webpage into a desktop app with Rust.  🤱🏻 利用 Rust 轻松构建轻量级多端桌面应用
- **[book-content-extraction](https://github.com/FB208/book-content-extraction)** - 暂无描述
- **[gfwlist2pac](https://github.com/FB208/gfwlist2pac)** - Automatically convert gfwlist to pac everyday
- **[hutool](https://github.com/FB208/hutool)** - A set of tools that keep Java sweet.
- **[PromptEngineering](https://github.com/FB208/PromptEngineering)** - OpenAI官方提示工程指南（升级版）配套代码
- **[mlink-tools](https://github.com/FB208/mlink-tools)** - 暂无描述
- **[FreeEhome](https://github.com/FB208/FreeEhome)** - 海康ehome开源服务
- **[WebClipperImages](https://github.com/FB208/WebClipperImages)** - 暂无描述
- **[winrar-keygen](https://github.com/FB208/winrar-keygen)** - Principle of WinRAR key generation.
- **[v2ray-core](https://github.com/FB208/v2ray-core)** - A platform for building proxies to bypass network restrictions.
- **[v2rayN](https://github.com/FB208/v2rayN)** - A GUI client for Windows, support Xray core and v2fly core and others
- **[md2all](https://github.com/FB208/md2all)** - 为防止md2all失效，fork了一份
- **[BigDuDuBlog](https://github.com/FB208/BigDuDuBlog)** - 暂无描述
- **[translation-agent](https://github.com/FB208/translation-agent)** - 吴恩达开源的翻译智能体
- **[PicGoImages](https://github.com/FB208/PicGoImages)** - 长期勿删
- **[WordCompare](https://github.com/FB208/WordCompare)** - word比对程序
- **[Shortcut-Key-Record](https://github.com/FB208/Shortcut-Key-Record)** - 手动记录快捷键，并根据当前活动窗口智能提醒
- **[rogsoft](https://github.com/FB208/rogsoft)** - software center for hnd/axhnd/axhnd.675x routers
- **[bigdudu](https://github.com/FB208/bigdudu)** - 暂无描述
- **[520](https://github.com/FB208/520)** - 暂无描述
- **[excel2json](https://github.com/FB208/excel2json)** - 把Excel表转换成json对象，并保存到一个文本文件中。
- **[waline](https://github.com/FB208/waline)** - 暂无描述
- **[Common.Utility](https://github.com/FB208/Common.Utility)** - Various helper class
- **[generator](https://github.com/FB208/generator)** - A code generator for MyBatis.
- **[SpringCloudAlibabaDemo](https://github.com/FB208/SpringCloudAlibabaDemo)** - 暂无描述
- **[AspNetCore.Docs](https://github.com/FB208/AspNetCore.Docs)** - Documentation for ASP.NET Core
- **[N_m3u8DL-CLI](https://github.com/FB208/N_m3u8DL-CLI)** - [.NET] m3u8 downloader 开源的命令行m3u8/HLS下载器，支持普通AES-128-CBC解密，多线程，自定义请求头等. 支持简体中文,繁体中文和英文.
- **[efo](https://github.com/FB208/efo)** - EFO是一个基于SpringBoot和Vue构建的文件分享系统，包括文件的上传与下载，文件的权限管理，远程文件管理等功能。
- **[TOOL](https://github.com/FB208/TOOL)** - 各种辅助工具、类


<!-- AUTO-GENERATED-PROJECTS-END -->

---

## 📊 GitHub统计

<div align="center">

![FB208's GitHub stats](https://github-readme-stats.vercel.app/api?username=FB208&show_icons=true&theme=vue-dark&hide_border=true&bg_color=0d1117)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=FB208&layout=compact&theme=vue-dark&hide_border=true&bg_color=0d1117&card_width=445)

</div>

---

## 💬 联系方式

- 📧 GitHub: [@FB208](https://github.com/FB208)
- ✉️ Email: [mark@agnet.top](mailto:mark@agnet.top)

---

<div align="center">

### ⚡ *"用代码提升效率，让工具服务生活"*

**如果你觉得我的项目有帮助，欢迎 Star ⭐**

![](https://komarev.com/ghpvc/?username=FB208&color=brightgreen&style=flat-square)

</div>